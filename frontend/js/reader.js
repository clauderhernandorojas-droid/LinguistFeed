...
export function displayTopicSelection() {
    ...
    const topics = [
        { id: 'homework', name: 'Reading Homework', icon: '📚' },
...
/**
 * 4. ARTÍCULO COMPLETO
 * Acepta un nivel opcional para recargar el artículo en otro nivel.
 */
async function loadFullArticle(id, level = null) {
    const container = document.getElementById('articles-container');
    if (!container) return;

    // Nivel activo: parámetro > localStorage > defecto B1
    ...
    try {
        // 1. INTENTAMOS OBTENER EL ARTÍCULO POR ID Y NIVEL desde /articles/:id?level=
        let article = null;
        try {
            const res = await fetch(`http://localhost:3001/articles/${id}?level=${currentLevel}`);
            if (res.ok) {
                article = await res.json();
            }
        } catch (_) { /* servidor offline, intentamos fallback */ }

        // 2. FALLBACK: buscamos en los artículos diarios
        if (!article) {
            const data = await fetchDailyArticles(currentLevel);
            article = (window.currentSimulationArticle && String(window.currentSimulationArticle.id) === String(id))
        ...
        const levelSelectorHTML = `
            <div id="level-selector-bar" style="
                display: flex; align-items: center; gap: 10px; flex-wrap: wrap;
                background: #f0f4f8; padding: 12px 16px; border-radius: 10px;
                margin: 20px 0; font-family: sans-serif;">
                <span style="font-weight: bold; color: #334e68; font-size: 0.9rem;">📖 Reading level:</span>
                ${levels.map(lvl => `
                    <button
                        onclick="window.changeArticleLevel('${id}', '${lvl}')"
                        style="padding: 6px 16px; border-radius: 20px; cursor: pointer; font-weight: bold;
        ...
        container.innerHTML = `
            <article style="max-width: 800px; margin: 0 auto; padding: 20px; font-family: sans-serif;">
                <a href="?topic=${(article.topic || 'world').toLowerCase()}" style="text-decoration:none; color:#007bff; font-weight:bold;">← Back to ${article.topic || 'Articles'}</a>
        ...
        // --- 2. RESALTADO PERSISTENTE DE FLASHCARDS GUARDADAS ---
        await highlightSavedFlashcards();

        // --- 3. ACTIVAR INTERACTIVIDAD DEL TEXTO ---
...
/**
 * Cambia el nivel del artículo actual y lo recarga.
 * Expuesto globalmente para los botones onclick del selector de nivel.
 */
window.changeArticleLevel = function(articleId, newLevel) {
    localStorage.setItem('user-level', newLevel);
    console.log(`🔄 Cambiando artículo ${articleId} a nivel ${newLevel}`);
    loadFullArticle(articleId, newLevel);
};

/**
 * RESALTADO PERSISTENTE
 * Obtiene las flashcards guardadas (servidor + localStorage como fallback)
 * y resalta las palabras que aparecen en el texto del artículo actual.
 */
async function highlightSavedFlashcards() {
    const textContainer = document.getElementById('interactive-text');
    if (!textContainer) return;

    // 1. Recopilar palabras guardadas — servidor primero, localStorage como fallback
    let savedWords = [];
    ...
    try {
        ...
        if (res.ok) {
            ...
            savedWords = serverCards.map(c => c.word).filter(Boolean);
            console.log(`✨ ${savedWords.length} flashcards obtenidas del servidor para resaltar.`);
        }
    } catch (_) {
        console.log('Servidor offline — usando localStorage para resaltado.');
    }

    // Fallback: localStorage
    if (savedWords.length === 0) {
        try {
            const local = JSON.parse(localStorage.getItem('linguistfeed_flashcards')) || [];
            savedWords = local.map(c => c.word).filter(Boolean);
        } catch (_) { savedWords = []; }
    }

    if (savedWords.length === 0) return;

    // 2. Resaltar cada palabra en el HTML del contenedor
    // Usamos regex con límites de palabra, case-insensitive
    ...
    savedWords.forEach(word => {
        ...
        html = html.replace(regex, `<mark style="background:#fff3bf; border-radius:3px; padding:0 2px;">$1</mark>`);
    });

    textContainer.innerHTML = html;
    console.log(`🖊️ Resaltado aplicado para: ${savedWords.join(', ')}`);
}
...
export async function saveFlashcardToStorage(injectedWord = null) {
    ...
    // Intento de servidor (ignorando el error si está offline)
...
// Exposición global (obligatorio para que el onclick del popup funcione)
